...
export default function Home() {
  ...
  const filter = () => {
    if(selected.toLowerCase() === 'all categories' || selected === 'Filter by'){
      return data
    } else {
      const filtered = data?.filter(item => item.category == selected.toLowerCase())
      return filtered
    }
  }
  ...
  return (
      ...
      <main className={styles.main}>
        ...
        <div className={styles.filterSection}>
          <div className={styles.filter} onClick={()=>setFilterDropdown(!filterDropdown)}>
            <p>{selected}</p>
            <svg transform={filterDropdown && "rotate(180)"} width="14" height="7" viewBox="0 0 14 9" fill="none" xmlns="http://www.w3.org/2000/svg">
